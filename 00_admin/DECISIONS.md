...
## ADR-014 - Opt-in provider-API delivery of the diagnose artifact (issue #18)
**Context:** `Invoke-CsmDiagnose.ps1` writes its report to the local `work_dir`; when the sync client itself is the broken part, the report cannot travel via that client. In the 2026-07-19 field incident, the report about a stuck upload queue was distributed via a direct Drive `files.create` call instead and was readable from a second device in seconds. B3 (offsite delivery of critical artifacts) and B14 (the sync client is part of the *target* environment; a diagnostic aimed at it should not depend on it for delivery) both point to a client-independent path. But the toolkit's standing boundary was "no data to cloud APIs beyond quota/metadata reads", so an upload path is a deliberate boundary change that must be recorded.
**Decision:** add `[diagnose_delivery]` (config-gated, **default off**): after writing the local artifact, ALSO upload that artifact — byte-identical as written, and redacted by construction (counts + health + root ordinals only) — via the provider's REST API to an operator-chosen folder ID. Credentials only as the NAME of an env var (A2/B13), never a value in config; the recommended scope is least-privilege `drive.file`. Delivery is one-way and **soft-fail** (B8): the classified outcome is recorded in the artifact and a local receipt, but a failed upload never fails the diagnostic. Per A7 a *transient* failure (network / HTTP 5xx) is retried with bounded exponential backoff (`[diagnose_delivery] max_attempts`/`retry_base_seconds`), while an auth/permission error (401/403/404) gives up immediately — a durable soft-failure, never an infinite retry. (The original #18 implementation was a single "one-shot" attempt; that did not satisfy A7's retry-with-backoff-but-give-up-on-auth requirement and was corrected before the v0.6.0 release — Part A outranks the "one-shot" framing this ADR first used.) Enabling the flag in local config is the operator's recorded standing approval (A1) for this enumerated action class. Google Drive first; other providers dispatch off `[diagnose_delivery] provider`. A test seam (`endpoint_base`) lets the smoke suite exercise the full upload against a local mock, so no real cloud is touched by tests.
**Consequence:** the diagnostic reaches the operator even when the client is jammed. The "no cloud writes" boundary now has exactly one enumerated, opt-in, redacted exception; everything else (inventory CSVs, hash baselines, log excerpts) stays local by scope decision. Rollback: disable the flag (config) or revert the commit.
